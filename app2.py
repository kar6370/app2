...
<style>
    ...
    .metric-card h2 { color: #1e293b !important; }
    .metric-card h3 { color: #475569 !important; font-size: 16px; }
    ...
    .notice-card h4 { color: #1e293b !important; }
    .notice-card p, .notice-card i, .notice-card b { color: #334155 !important; }
    ...
    .guide-step h3, .guide-step h4, .guide-step p, .guide-step b {
        color: #1e293b !important;
    }
...
# 2. 전역 변수 및 챌린지 룰 세팅 
# ==========================================
USERS = ["김효진", "김경민", "김승섭", "이승주", "신호성", "최소연", "김재균", "정진교", "장명광", "원영길", "김미야", "한승희", "이순학"]
...
# ==========================================
# 4. 데이터 처리 헬퍼 함수 (읽기 초과 방지용 캐시 적용)
# ==========================================

# 💡 [핵심 최적화] 데이터를 한 번 불러오면 10분(600초) 동안 기억하여 DB 호출 최소화
@st.cache_data(ttl=600, show_spinner=False)
def fetch_data_from_firestore():
    if db is None:
        return None
    try:
        # 타임아웃 5초 설정: 한도 초과 시 무한 로딩(재시도)에 빠지지 않도록 함
        docs = db.collection("walking_challenge").stream(timeout=5)
        return [doc.to_dict() for doc in docs]
    except Exception as e:
        # ResourceExhausted 에러 또는 타임아웃 발생 시 잡아냄
        return None 
...
def save_steps(name, department, record_date_obj, month, steps):
    ...
    if db:
        try:
            ...
            # 새로운 데이터가 입력되었으므로 기억해둔 캐시를 지우고 새로고침 유도
            fetch_data_from_firestore.clear()
        except:
            st.error("🚨 현재 앱 접속량이 많아 일시적으로 저장이 지연되고 있습니다. 오늘 오후 4~5시 이후에 다시 시도해 주세요! (기존 데이터는 안전합니다)")
    else:
...
def get_all_data():
    if db:
        ...
        if data is None:
            # 5만회 한도 초과 등 에러 발생 시 빈 데이터프레임 반환
            st.error("🚨 현재 일일 시스템 접속량이 초과되어 일시적으로 화면을 불러올 수 없습니다.\n\n(참여자분들의 기존 기록은 아주 안전하게 보관되어 있으며, 오늘 오후 4~5시경 자동으로 정상 복구됩니다. 잠시 후 다시 방문해 주세요!)")
            df = pd.DataFrame(columns=["name", "department", "record_date", "month", "steps", "timestamp"])
        else:
...
def delete_record_by_date(name, record_date_str):
    if db:
        try:
            # 여기에도 타임아웃 5초를 추가하여 무한 로딩 방지
            docs = db.collection("walking_challenge").where("name", "==", name).stream(timeout=5)
            for doc in docs:
                doc_dict = doc.to_dict()
                if doc_dict.get("record_date") == record_date_str:
                    doc.reference.delete()
            # 데이터가 삭제되었으므로 기억해둔 캐시 초기화
            fetch_data_from_firestore.clear()
        except:
            st.error("🚨 현재 앱 접속량이 많아 일시적으로 삭제가 지연되고 있습니다. 오늘 오후 4~5시 이후에 다시 시도해 주세요!")
    else:
...
menu = st.sidebar.radio("📌 메뉴 이동", ["📢 공지사항", "📖 걸음 수 확인 가이드", "👟 걸음 수 입력", "🏃‍♂️ 워밍업 (사전연습)", "📊 내 대시보드", "🏆 전사 리더보드"])
...
if not df_all.empty:
    ...
    df_official = df_all[df_all['month'] >= 3].copy()
    df_warmup = df_all[df_all['month'] == 2].copy()
else:
...
if menu == "📢 공지사항":
    ...
    with col1:
        ...
        <div class="notice-card">
            <h4 style="color:#2563EB !important;">🏅 1. 부서 워킹 대항전</h4>
            <p><b>평가방식:</b> 참여율(70점) + 3~5월 총 걸음수(30점)<br>
            <i>※ 각 그룹당 우수부서 선정 및 포상</i></p>
        </div>
        <div class="notice-card">
            <h4 style="color:#D97706 !important;">👑 2. 워킹 King</h4>
    ...
    with col2:
        st.markdown("""
        <div class="notice-card">
            <h4 style="color:#059669 !important;">⭐ 3. 만보 달성자</h4>
            <p><b>선정기준:</b> 3~5월 중 <b>월 평균 10,000보 이상 달성자</b><br>
            <i>※ 달성자 중 매월 50명 무작위 추첨</i></p>
        </div>
        <div class="notice-card">
            <h4 style="color:#9333EA !important;">🎁 4. 행운상</h4>
    ...
    st.markdown("""
    <div style="background-color: white; padding: 20px; border-radius: 10px; border-left: 5px solid #10b981; margin-bottom: 20px;">
        <p><span class="timeline-date">📍 2026. 2. 19. ~ 2. 28.</span><br>워킹 챌린지 홍보 및 <b>사전 연습(앱 테스트)</b></p>
        <p><span class="timeline-date">📍 2026. 3. 01. ~ 5. 31.</span><br><b>워킹 챌린지 본격 실시</b> (3개월간 본 경기 진행)</p>
        <p><span class="timeline-date">📍 2026. 6. 한</span><br>결과보고 및 참여자 상품 지급</p>
    </div>
    """, unsafe_allow_html=True)
...
# ------------------------------------------
    ...
    today = dt.date.today()
    warmup_start = dt.date(2026, 2, 26)
    end_date = dt.date(2026, 5, 31)
    
    max_allowed_date = min(today, end_date)
    if max_allowed_date < warmup_start:
        max_allowed_date = warmup_start
        st.warning(f"⏳ 아직 앱 오픈 전({warmup_start.strftime('%m/%d')} 이전)입니다. 현재는 강제 테스트 모드입니다.")
    ...
    with st.form("step_form"):
        ...
        if submit:
            if record_date > today and record_date > warmup_start:
            ...
            if input_month == 2:
                st.success(f"🎉 [사전연습] {name}님의 {record_date.strftime('%m월 %d일')} 걸음 수({steps:,}보)가 등록되었습니다! (공식 기록 미합산)")
                st.balloons()
            else:
...
# [페이지 1-2] 워밍업 (사전연습)
# ------------------------------------------
elif menu == "🏃‍♂️ 워밍업 (사전연습)":
    st.markdown('<div class="sub-title">🏃‍♂️ 사전연습 (워밍업) 기록</div>', unsafe_allow_html=True)
    st.info("💡 안내: 2월 26일~28일의 테스트 기록입니다. 공식 순위에는 반영되지 않습니다.")
    
    if df_warmup.empty:
        st.warning("등록된 연습 기록이 없습니다.")
    else:
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("#### 🎯 워밍업 참가 현황")
            summary = df_warmup.groupby('name')['steps'].sum().reset_index().sort_values('steps', ascending=False)
            summary.columns = ['성명', '총 연습걸음']
            st.dataframe(summary, use_container_width=True, hide_index=True)
        with col2:
            st.markdown("#### 📅 상세 로그")
            st.dataframe(df_warmup[['record_date', 'name', 'steps']].sort_values('record_date', ascending=False), use_container_width=True, hide_index=True)

        st.markdown("---")
        st.markdown("#### 🗑️ 연습 기록 삭제 (관리)")
        del_user = st.selectbox("본인 성명을 선택하세요", ["선택하세요..."] + USERS, key="warmup_del_user")
        if del_user != "선택하세요...":
            user_warmup = df_warmup[df_warmup['name'] == del_user].sort_values('record_date', ascending=False)
            if not user_warmup.empty:
                del_opt = ["선택 안 함"] + user_warmup.apply(lambda x: f"{x['record_date']} ({int(x['steps']):,}보)", axis=1).tolist()
                target = st.selectbox("삭제할 연습 기록 선택", del_opt, key="warmup_del_target")
                if target != "선택 안 함":
                    if st.button("🚨 해당 연습 기록 삭제", type="primary"):
                        delete_record_by_date(del_user, target.split(" ")[0])
                        st.success("연습 기록이 삭제되었습니다.")
                        st.rerun()
            else:
                st.write("삭제할 연습 기록이 없습니다.")
...
# ------------------------------------------
    ...
    if search_name and not df_all.empty:
        user_data_official = df_official[df_official['name'] == search_name].copy()
        user_data_all = df_all[df_all['name'] == search_name].copy()
        
        if not user_data_official.empty:
            user_monthly = user_data_official.groupby('month')['steps'].sum().reset_index()
            total_steps = user_monthly['steps'].sum()
            
            last_month = user_monthly['month'].max()
            last_month_data = user_data_official[user_data_official['month'] == last_month]
            ...
            col1.markdown(f'<div class="metric-card"><h3>총 누적 걸음(본경기)</h3><h2>{total_steps:,}보</h2></div>', unsafe_allow_html=True)
            ...
            st.warning("등록된 '공식(3~5월)' 데이터가 없습니다. 워밍업 기록은 [워밍업] 메뉴에서 확인하세요.")

        st.markdown("---")
        st.markdown('#### ⚙️ 내 기록 관리 (삭제)')
        st.caption("💡 잘못 입력한 날짜의 기록을 선택하여 삭제할 수 있습니다.")
        user_records_sorted = user_data_all.sort_values('record_date', ascending=False)
...
# ------------------------------------------
    ...
    st.markdown('<div class="sub-title">3. 챌린지 랭킹 및 통계 (본 경기)</div>', unsafe_allow_html=True)
    st.info("💡 이 곳의 통계는 **3월 1일부터 입력된 공식 기록**만 반영됩니다.")
    
    if df_official.empty:
        st.warning("아직 등록된 공식 데이터가 없습니다. (3월 1일부터 집계됩니다)")
    else:
        user_total = df_official.groupby(['name', 'department'])['steps'].sum().reset_index().sort_values('steps', ascending=False)
        
        monthly_record_days = df_official.groupby(['name', 'department', 'month'])['record_date'].nunique().reset_index()
        monthly_record_days.rename(columns={'record_date': 'recorded_days'}, inplace=True)
        
        monthly_user = df_official.groupby(['name', 'department', 'month'])['steps'].sum().reset_index()
            ...
            for i in range(min(3, len(top3))):
                with cols[i]:
                    ...
                    <div class="metric-card" style="border-top: 5px solid #F59E0B;">
                        <h3>{medals[i]}</h3>
                        <h2 style="color:#D97706 !important;">{top3.loc[i, 'name']}</h2>
                        <p>{top3.loc[i, 'department']}</p>
                        <h4 style="color:#1e293b !important;">{int(top3.loc[i, 'steps']):,} 보</h4>
                    </div>
                    """, unsafe_allow_html=True)
